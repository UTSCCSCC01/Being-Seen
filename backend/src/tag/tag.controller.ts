...
export class TagController {
  ...
  async getTagList(@Body('tagList') tagList: string[]) {
    const list = await this.tagService.getListOfTags(tagList);
    return list;
  }
  /*
    @Put("/:tagName")
  ...
  async createTagList(@Body('tagList') tagList: string[]) {
    const list = await this.tagService.createTagList(tagList);
    return list;
  }
  /**
  ...
  async editTag(
    ...
    const result = this.tagService.editTagById(tagId, tagName);
    return result;
  }
}
